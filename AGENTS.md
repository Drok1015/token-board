...
- 自动更新基于 tauri-plugin-updater：默认开启（设置页可关闭），开启时应用启动及每 2 小时读取 `releases/latest/download/latest.json`，有新版即后台安装并重启；安装失败时弹系统对话框引导用户手动解除 quarantine；关闭时不做任何自动检查，右键菜单「检查更新」仍可手动触发。签名私钥存于仓库 secret `TAURI_SIGNING_PRIVATE_KEY`（本地备份 `~/.tauri/token-board.key`，未入 Git），公钥内置 `tauri.conf.json`；本地打包需同时导出 `TAURI_SIGNING_PRIVATE_KEY` 和 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD=""`。
