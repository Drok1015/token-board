...
- 代码改动 push 到 `main` 即自动发版：CI（`.github/workflows/release.yml`）把最新 tag 的 patch 版本 +1，由机器人提交 `chore: bump version` 回写 main（同步 5 个版本文件），随后完成 macOS 构建、ad-hoc 签名并发布 Release（`TokenBoard-v<版本>-macos.zip`，Latest）。
